...
class debugger():
    ...
    def bp_set_hw(self, address, length, condition):
        ...
        # We want to set the deubg register in every thread
        for thread_id in self.enumerate_threads():
            ...
            # Set the breakponit condition
            context.Dr7 |= condition << ((available * 4) + 16)

            # Set the length
            context.Dr7 |= length << ((available * 4) + 18)

            # Set this thread's context with the debug registers set
    ...
    def exception_handler_single_step(self):
        ...
        # isn't properyl propagating that flag down to us.
        if self.context.Dr6 & 1 and self.hardware_breakpoints.has_key(0):
            slot = 0
        elif self.context.Dr6 & 0x2 and self.hardware_breakpoints.has_key(1):
            slot = 1
        elif self.context.Dr6 & 0x4 and self.hardware_breakpoints.has_key(1):
            slot = 2
        elif self.context.Dr6 & 0x8 and self.hardware_breakpoints.has_key(1):
            slot = 3
        else:
    ...
    def bp_set_mem(self, address, size):
        ...
        while current_page <= address + size:
            ...
            # differenriate our guarded pages from those
            ...
            # Instead our range by the size of the
            # default system memory page size
            current_page += self.page_size

        # Add the memory breakpoint to our global list
        self.memory_breakponts[address] = (address, size, mbi)
